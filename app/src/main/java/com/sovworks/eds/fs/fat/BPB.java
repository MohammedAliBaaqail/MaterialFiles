...
class BPB {
    ...
    void read(RandomAccessIO input) throws IOException, WrongImageFormatException {
        ...
        numberOfFATs = (short) Util.readUnsignedByte(input);
        rootDirEntries = Util.readWordLE(input);
        totalSectorsNumber = Util.readWordLE(input);
        mediaType = (short) Util.readUnsignedByte(input);
    ...
    void calcParams() {
        _clusterOffsetStart = (long) bytesPerSector * (reservedSectors + sectorsPerFat * numberOfFATs) + rootDirEntries * 32;
    }

    protected long _clusterOffsetStart;

    protected void readCommonPart(RandomAccessIO input) throws IOException {
        physicalDriveNumber = (short) Util.readUnsignedByte(input);
        input.read();
        extendedBootSignature = (short) Util.readUnsignedByte(input);
...
class BPB32 extends BPB {
    ...
    @Override
    void calcParams() {
        _clusterOffsetStart = (long) bytesPerSector * (reservedSectors + sectorsPerFat32 * numberOfFATs);
    }
}
