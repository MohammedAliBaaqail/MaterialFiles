...
public class ExFat implements FileSystem
{
    ...
    private static void tryImportModule() {
        String[] searchPaths = {
                "/sdcard/Download/libedsexfat.so",
                "/sdcard/Download/edsexfat.so",
                "/sdcard/EDS/libedsexfat.so",
                "/sdcard/EDS/edsexfat.so",
                "/sdcard/libedsexfat.so",
                "/storage/emulated/0/Download/libedsexfat.so",
                "/storage/emulated/0/Download/libedsexfat-v1.2-arm64-v8a.so",
                "/storage/emulated/0/EDS/libedsexfat.so",
                "/storage/emulated/0/libedsexfat.so"
        };
        File target = getModulePath();
        if (target.exists()) {
            Logger.debug("ExFAT module found in private storage.");
            return;
        }
        ...
        for (String path : searchPaths) {
            ...
            if (source.exists()) {
                Logger.debug("Found ExFAT module at " + path + ". Importing...");
                try {
                    copyFile(source, target);
                    Logger.debug("ExFAT module imported successfully to " + target.getAbsolutePath());
                    break;
                } catch (IOException e) {
                    Logger.debug("Failed to import ExFAT module from " + path + ": " + e.getMessage());
                }
            }
        }
        
        if (!target.exists()) {
            Logger.debug("ExFAT module not found in common locations. Please place libedsexfat.so in your Download folder.");
        }
    }
